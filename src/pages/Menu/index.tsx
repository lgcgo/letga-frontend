...
const Page: React.FC = () => {
  ...
  const handleSetStatus = async (record: DataType, value: 'normal' | 'disabled') => {
    await waitTime(200);
    try {
      await MenuService.api.setStatus({ key: record.key, value: value });
  ...
  const handleRemove = async (selectedRows: DataType[]) => {
    ...
    try {
      ...
      await MenuService.api.remove(keys);
      message.success(intl.formatMessage({
  ...
  // 深度优先遍历转换
  const loopConvert = (data: API.MenuTreeNode): DataType => {
  ...
  const fetchDataSource = () => new Promise((resolve) => {
    MenuService.api.getTree({ search: search.current }).then((res) => {
      ...
      <Row gutter={[24, 24]}>
        <Col span={24}>
          <PageHeader
            ...
            title={<FormattedMessage id="pages.menus.title" />}
            extra={[
              <SearchInput
                key='ph-search'
                placeholder={
                  intl.formatMessage({
                    id: 'pages.menus.search.placeholder',
                  })
                }
