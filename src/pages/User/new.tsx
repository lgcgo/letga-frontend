...
const Page: React.FC = () => {
  ...
  const handleSubmit = async (fields: API.User) => {
    await waitTime(300);
    try {
      await UserService.api.create(fields);
      ...
      <Row gutter={[24, 12]}>
        <Col span={24}>
          <PageHeader
            ...
            title={<Space><ArrowLeftOutlined onClick={() => navigate(-1)} /><FormattedMessage id='pages.users.title.new' /></Space>}
          />
        </Col>
              ...
              <ProCard title={<FormattedMessage id="form.user.title.loginInfo" />}>
                ...
                <ProFormText
                  ...
                  rules={[
                    {
                      required: true,
                      message: <FormattedMessage id="form.user.rules.password.required" />,
                    },
                    {
                      min: 6,
