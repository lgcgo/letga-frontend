...
const NewPage: React.FC = () => {
  ...
  const handleSubmit = async (fields: API.AuthRoute) => {
    await waitTime(300);
    try {
      await AuthService.route.update(fields);
      ...
      <Row gutter={[24, 12]}>
        <Col span={24}>
          <PageHeader
            ...
            title={<Space><ArrowLeftOutlined onClick={() => navigate(-1)} /><FormattedMessage id='pages.auth.routes.title.update' /></Space>}
          />
        </Col>
        <Col md={{ span: 24 }} lg={{ span: 20, offset: 1 }} xl={{ span: 16, offset: 3 }}>
          <ProCard title={<FormattedMessage id='form.auth.route.card.title' />}>
            <ProForm<API.AuthRoute>
              request={() => AuthService.route.get({ params: { key: params.id } })}
              labelCol={{ span: 6 }}
              wrapperCol={{ span: 11 }}
              layout="horizontal"
              colon={true}
